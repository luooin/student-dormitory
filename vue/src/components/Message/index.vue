<template>
  <div>
            ...
            <div style="width: 80%;margin: 0 auto">
              <el-table :data="pageInfo.list" height="470">
                ...
                <el-table-column label="内容" width="400">
                  <template slot-scope="scope">{{ scope.row.msg.substr(0, 25) }}{{ scope.row.msg.length > 25 ? "..." :
                    ""}}</template>
                </el-table-column>
                  ...
                  <template slot-scope="scope">
                    <template>
                      ...
                      <el-popconfirm title="确定删除吗？" @confirm="msgDel(scope.row.id, 1)">
                        <el-button style="margin-left: 8px" slot="reference" size="mini" type="danger">删除
                        </el-button>
                      </el-popconfirm>
                    </template>
                  </template>
              ...
              <div class="page" style="margin-top: 10px">
                <el-pagination background :hide-on-single-page="false" :page-size="listQuery.rows"
                  :page-count="pageInfo.pages" layout="->, prev, pager, next, jumper, total, sizes"
                  :total="pageInfo.total" :current-page.sync="pageInfo.pageNum" @current-change="handleCurrentChange"
            ...
            <div style="width: 80%;margin: 0 auto">
              <el-table :data="pageInfo.list" height="470">
                ...
                <el-table-column label="内容" width="320">
                  <template slot-scope="scope">{{ scope.row.msg.substr(0, 35) }}{{ scope.row.msg.length > 35 ? "..." :
                    ""}}</template>
                </el-table-column>
                <el-table-column label="时间" prop="time" width="200"></el-table-column>
                <el-table-column label="操作" width="200">
            ...
            <div style="width: 90%;margin: 0 auto">
              <el-table :data="pageInfo.list" height="470">
                <el-table-column label="学生姓名">
                  <template slot-scope="scope">
                    <el-popover placement="bottom" width="200" trigger="click"
                      @show="getStudent(scope.row.application.student.id)" @hide="hidePopover">
                      ...
                      <span slot="reference"
                        style="color: #409EFF;cursor: pointer">{{ scope.row.application.student.name }}</span>
                    </el-popover>
                  </template>
                </el-table-column>
                ...
                <el-table-column label="操作">
                  <template slot-scope="scope">
                    <el-button type="primary" size="mini" :disabled="scope.row.isAgree !== undefined"
                      @click="showVerifyDialog(scope.row.id)">审核</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </div>
          ...
          <el-tab-pane label="我发起的" name="send">
            <div style="width: 80%;margin: 0 auto">
              <el-table :data="pageInfo.list" height="470">
                <el-table-column label="学生姓名">
                  <template slot-scope="scope">
                    <el-popover placement="bottom" width="200" trigger="click" @show="getStudent(scope.row.student.id)"
                      @hide="hidePopover">
                      <el-form label-width="40px">
                        <el-form-item label="姓名">
                          <el-input v-model="student.name" readonly size="mini"></el-input>
                ...
                <el-table-column label="操作">
                  <template slot-scope="scope">
                    <el-button size="mini" type="primary"
                      @click="showDepartVerifyDialog(scope.row.id)">查看审核记录</el-button>
                    <el-button size="mini" type="danger" :disabled="scope.row.isFinished"
                      @click="msgDel(scope.row.id, 3)">撤销申请</el-button>
                  </template>
                </el-table-column>
              </el-table>
              <div class="page" style="margin-top: 10px">
        ...
        <div style="width: 80%;margin: 20px auto">
          <el-form :model="messageToSend" ref="sendMsg" :rules="messageToSendRules" label-width="100px">
            <el-form-item label="接收宿舍楼" prop="to">
              <tree-select @open="listBuildingList" v-model="messageToSend.to" :options="buildingList"
                :normalizer="normalizer" :show-count="true" placeholder="选择宿舍楼">
              </tree-select>
            </el-form-item>
    ...
    <div class="depart-dialog">
      <el-dialog :visible.sync="departVerifyDialogVisible" @closed="departVerifyDialogClose" append-to-body title="审核记录"
        width="40%">
        <el-timeline>
          <el-timeline-item v-for="(activity, index) in departVerifyList" :key="index"
            ...
            {{ activity.operateUser.realName + "：" +
              (activity.isAgree === undefined ? "等待审核" : ((activity.isAgree ? "通过" : "不通过") + "，原因：" +
                (activity.reason ||"无")))}}
          </el-timeline-item>
        </el-timeline>
      </el-dialog>
